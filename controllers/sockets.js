...
const webRTCSignalingSocket = (io) => {
    io.on("connection", (socket) => {
        ...
        socket.on("prepare-session", async ({ sessionId, userId }) => {
            ...
            if (session) {
                ...
                socket.on("disconnect", async () => {
                    console.log(`Observer ${userId} disconnected from session ${sessionId}`);
                });
            } else {
        ...
        socket.on("join-session", async ({ sessionId, userId, name, photo, micOn, videoOn }) => {
            console.log(`User ${name} (${userId}) is attempting to join session ${sessionId}`);
            ...
            if (session) {
                ...
                if (existingParticipantIndex !== -1) {
                    session.participants[existingParticipantIndex] = {
                        ...
                        photo: photo || session.participants[existingParticipantIndex].photo,
                        micOn: micOn,
                        videoOn: videoOn,
                        socketId: socket.id,
                    };
                ...
                console.log(`User ${name} (${userId}) has joined session ${sessionId}`);
        ...
        socket.on("send-offer", async ({ sessionId, sender, receiver, offer }) => {
            console.log(`User ${sender} is sending an offer ${receiver} to session ${sessionId}`);
            io.to(sessionId).emit("receive-offer", { sender, receiver, offer });
        });

        socket.on("send-answer", async ({ sessionId, sender, receiver, answer }) => {
            console.log(`User ${sender} is sending an answer to ${receiver} session ${sessionId}`);
        ...
        socket.on("send-ice-candidate", async ({ sessionId, sender, receiver, candidate }) => {
            console.log(`User ${sender} is sending ICE candidate to ${receiver} session ${sessionId}`);
        ...
        socket.on("toggle-video", async ({ sessionId, userId }) => {
            ...
            if (session) {
                ...
                if (participant) {
                    ...
                    await session.save();
                    console.log(
                        `User ${userId} has turned their video ${participant.videoOn ? "on" : "off"}`
                    );
                    io.to(sessionId).emit("participant-update", participant);
                }
            }
